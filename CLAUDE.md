...
scripts/
  ...
  push_notion.py       upsert one Notion row per paper / news item (env NOTION_TOKEN/NOTION_DB_ID)
data/
...
digests/<date>.json         the digest deliverable + durable readable record, authored by Claude
...
news/                        SECOND TRACK — policy / market news, daily, Claude-run, no scripts
...
Set `$env:JOURNAL_RESEARCH_EMAIL` (a real contact address) before running
`fetch_feeds.py` / `enrich.py` — used as the polite-pool mailto for
Crossref/OpenAlex; falls back to a placeholder if unset.

## Weekly workflow

Operational runbook: `.claude/skills/material-intel/SKILL.md` (Track A: A0 tune →
...
5. **Score & write the digest** — done by Claude, not a script:
...
Authored by Claude as `digests/<date>.json`; `render_site.py` turns it (plus the
...
script — Claude sweeps the sources live. Reports are in Japanese.
