...
- **Track A — paper digest** (weekly): the polymer / materials-chemistry journal
  digest. Design + rubric: `CLAUDE.md` and `config/scoring.md`.
- **Track B — policy / market news** (daily): `CLAUDE.md` > "News track" +
  `news/principles.md`, `news/curation_notes.md`, `news/sources.md`.
- Both finish with **Track C — render + publish** the combined page, then log
  each paper / news item as its own row in a Notion database (`push_notion.py`).
...
- Combined Artifact (reuse this URL, never make a new one):
  `https://claude.ai/code/artifact/93c92274-daa7-4e29-a86a-e1bf024efbbc`
...
5. **Write** — author `digests/<date>.json` per the schema in `CLAUDE.md`.
...
4. **Log to Notion — one row per paper / news item** —
   `python scripts/push_notion.py <date>`
   Upserts a row per digest pick, per digest watchlist entry, and per news item
   (digest rows when `digests/<date>.json` exists, news rows when
   `news/<date>_policy_news.json` exists). Idempotent on the `キー` property
   (`paper:<doi>` / `news:<date>:<id>`) — re-runs update, never duplicate.
   Columns filled: タイトル / 種別(論文·ニュース) / 日付 / 掲載号 / 区分(Pick·
   Watchlist·本筋·裾野) / スコア / 出典 / テーマ / リンク / 評価(feedback.md の
   +·~·-) / 概要 / キー. Manual columns (確認状況·重要度·メモ) are never touched.
   - Needs env vars `NOTION_TOKEN` and `NOTION_DB_ID` (internal integration token
     + the DB shared with / created under a page shared with that integration).
     If unset, the script exits with a message — tell the user and skip for now.
   - `--dry-run` prints the rows; `--kind digest|news` forces one.
